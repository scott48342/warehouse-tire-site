# H5 Ã¢â‚¬â€ Minimum Load Index: provenance write-up (DECISION NEEDED)

Audit: `website-fitment-audit-2026-09-18.md` Ã‚Â§ H5. Evidence gathered 2026-09-18 22:15 ET, read-only
...
`serviceSpecs.oemLoadIndex` Ã¢â€ â€™ `vehicle_fitments.oem_load_index`, filled DB-wide on **2026-09-17** by
...
| `usaf-max` | 1,448 | same size listed twice at USAF with different LIs Ã¢â€ â€™ higher one kept (Scott, 09-17) |
...
Total 38,510 rows; 28,395 have an LI (73.7%). Provenance is therefore **not unclear** Ã¢â‚¬â€ it is
...
1. **Option sizes (same axle, different trims/packages)** Ã¢â‚¬â€ e.g. 2016 Fusion S `215/60R16, 225/50R17`
   LI 95. Usually close, occasionally 2Ã¢â‚¬â€œ4 points apart. Low risk, but the value is still "one of the OE
   sizes", not "the minimum for this trim".
2. **Staggered (front Ã¢â€°Â  rear)** Ã¢â‚¬â€ the value is *front-only*. Verified live:
...
- `requiredLoadIndexSource` is **always** `vehicle_record_unverified`; `VERIFIED_LOAD_SOURCES`
  (`verified_manufacturer`) is never emitted. So no tire is ever badge-certified on load index Ã¢â‚¬â€ the
...
`oem_load_index_by_size jsonb` (`{"245/35R19":89,"305/30R20":103}`) Ã¢â‚¬â€ a Drizzle migration per the WTD
migration skill Ã¢â‚¬â€ and backfill from the cached responses (no new supplier calls for rows already
...
Rows affected by the re-stamp: Ã¢â€°Â¤13,946 Ã¢â€ â€™ **>100 rows, needs your OK**.
...
- The `verified_manufacturer` path stays unreachable until a source you sign off on (TG Pro feed in
  writing, or manufacturer placards) is wired Ã¢â‚¬â€ USAF is a distributor catalog, not a manufacturer.

---

## Option A â€” PREPARED FOR REVIEW (2026-09-19, nothing applied)

| Piece | Where | State |
|---|---|---|
| Migration (additive) | `drizzle/migrations/0051_fitment_load_index_by_size.sql` (fix branch `77d20e48`) | not applied |
| Rollback | `drizzle/migrations/0051_rollback.sql` | â€” |
| Backfill, dry-run first | `scripts/audit/pass5/h5-load-index-by-size/dry-run.mjs` â€” reads ONLY the USAF payloads cached 2026-09-17 (main checkout, read-only); `--apply` refuses without `--approved-by scott` **and** 0051; never touches the scalar; snapshots to `audit_h5_load_index_by_size_original` | dry run done |
| Runtime read path | branch **`h5-option-a-runtime`** `f65f0e9a` (worktree `F:\clawd\wt-h5-runtime`) â€” schema, `serviceSpecs.oemLoadIndexBySize`, cache v8, `tires/search` scope `per_size` | tsc 0, **not** on the deploy branch |
| Interim (already on fix branch) | `09e18766` â€” scope `per_axle_unknown` fails closed | live on :3002 |

### Dry-run numbers (`out/summary.json`, `out/diff.csv` local)
- 32,740 live rows â†’ **28,268** would get a per-size map (27,774 complete, 494 partial â€” a size USAF lacks).
...
- Result with an exact OE size â†’ that size's own OE LI.
- OE-staggered vehicle, result on a rear-only rim diameter â†’ min over rear OE options; front-only â†’ front; ambiguous â†’ the higher axle.
- Otherwise â†’ min over OE options. `requiredLoadIndexSource` stays `vehicle_record_unverified` (USAF is a distributor catalog) â†’ still no load-index badge; `packageEligible` returns to true for tires that meet their own requirement.
- Rows without a complete map â†’ unchanged interim behaviour (`per_axle_unknown`, package-ineligible, no minimum shown).

### Exact procedure when you approve (no step runs without your go)
1. `node scripts/apply-fitment-migration.mjs drizzle/migrations/0051_fitment_load_index_by_size.sql` (or the repo's usual `scripts/apply-*.mjs` runner against `POSTGRES_URL`) â€” additive, safe with the current deploy.
2. `node --env-file=.env.local scripts/audit/pass5/h5-load-index-by-size/dry-run.mjs` â€” re-check `out/summary.json` matches the numbers above.
3. `node --env-file=.env.local scripts/audit/pass5/h5-load-index-by-size/dry-run.mjs --apply --approved-by scott` â€” 28,268 rows, one transaction, snapshot table first. Verify: `select count(*) from vehicle_fitments where oem_load_index_by_size is not null` = 28,268.
4. Merge `h5-option-a-runtime` (`f65f0e9a`) into the deploy branch; deploy. Cache key bumps to v8 automatically.
5. Optional, separate decision: re-stamp scalar `oem_load_index = max(by_size)`, source `usaf-max` (â‰¤12,188 rows) so the single displayed number is never under an axle's requirement.
