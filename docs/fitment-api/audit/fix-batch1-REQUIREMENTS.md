# Fix Batch 1 â€” CONSOLIDATED REQUIREMENTS (authoritative, 2026-09-18 13:58)
...
3. Bump `CACHE_VERSION` v5 â†’ v6 in fitmentCache.ts. Grep for any other cache keys/versions that store resolver/profile/tire-search/package output (key prefixes containing fitment / profile / vehicle / tires / package) and bump or namespace those that carry fields changed by this batch. List every key/version touched in the handoff.
...
## R2. Exact model resolution (F5) â€” verify existing commit e2211ac6
- No substring path may cross models. "2024 Ford Mustang" â†’ 5x114.3 / 70.5; "Mustang Mach-E" â†’ 5x108 / 63.4. Aliases keep working (Silverado 2500 HDâ†’2500HD, F-250â†’Super Duty, Mercedesâ†’Mercedes-Benz).
...
## R3. Trim ambiguity (F7) â€” REWRITE `src/lib/fitment-db/trimAmbiguity.ts`
...
- Differing wheel width / diameter set / offset / axle config / load requirement â†’ trim required even when tire-size strings match.
...
- Auto-select without trim only when all compared fields `agree`, or single trim / grouped record (existing behaviour) â€” and grouped records must still be checked field-by-field, not assumed.

### check-fitment (F7/F13) â€” fix regression in c684a1e0
- Current code sets `trimRequiredNote`, skips geometry, and can return `fits: true` on shared bolt pattern. FORBIDDEN.
- No trim + constraints unresolved â†’ `fits: null`, `reason: "trim_required"`, optionally `boltPatternCompatible: true|false`. Bolt disagrees â†’ `trim_required`. Bolt agrees but width/offset/dia disagree â†’ `fits: null`, `reason: "trim_required_for_geometry"`, `boltPatternCompatible: true`. Never overall `fits: true` from partial agreement.
- Existing negative cases unchanged: bolt mismatch rejected; Fuel `D68117906545` on 2024 Camry â†’ `geometry_rejected`.

### Universal resolver consumers
`universalFitmentResolver` still auto-selects a trim from metadata. Grep every consumer (fitment-search, tire-sizes, packages/recommended engine.ts, tires/search, check-fitment, Jake, POS, staggered-search, plus-sizes, anything else importing it) and make sure none bypasses the ambiguity gate: either route auto-select through trimAmbiguity, or have the resolver return `trimRequired`/`certifiable:false` and make each consumer honour it (packages â†’ no certified packages; fitment-search may browse on sharedSpecs with NO fit badge). List each consumer and its disposition in the progress file.

## R4. Load-index gating (F3) â€” REWRITE `src/lib/tires/loadIndexGate.ts` (draft in 81959ba8 is wrong)
...
- The vehicle-record minima (Raptor 119, M4 100, â€¦) are NOT verified OEM and NOT axle-specific. Never label them OE/OEM/factory in UI or API. Fields: `requiredLoadIndex`, `requiredLoadIndexSource` (e.g. `"vehicle_record_unverified"`), `tireLoadIndex`, `loadIndexOk: true|false|null`, `loadIndexChecked`.
- Tire below required â†’ EXCLUDE from recommended/compatible packages and BLOCK every fit-certified package/cart construction path with reason code `load_index_below_required`. Generic unverified browsing MAY show it with a clear message ("Load rating 110 is below the 119 this vehicle requires") and NO fit badge.
- Missing or unverified minimum â†’ `fitBadgeAllowed: false`. A green/Guaranteed/Verified badge appears ONLY when a requirement exists AND the tire meets it. (The draft's "missing minimum â‡’ fitBadgeAllowed:true" is the opposite and must be inverted.)
...
- Unit (pure helpers, fixtures): modelMatch; trimAmbiguity cases (a) same tires / different width-offset â†’ trim_required; (b) same tires / different requiredLoadIndex â†’ trim_required; (c) known+unknown bolt â†’ unknown, no auto-resolve, no badge; (d) full agreement â†’ auto-resolve; (e) staggered vs square with overlapping front size â†’ trim_required; (f) >limit candidate rows with the conflicting one last â†’ trim_required. loadIndexGate: 110<119 â†’ blocked; 99<100 â†’ blocked; equal â†’ ok; missing requirement â†’ no badge.
- Endpoint/integration (repo's route-test pattern; mock DB layer or fixtures; assert response shape + reason codes):
  `/api/vehicles/search` 2024 Ford Mustang â†’ 5x114.3 not Mach-E; Mach-E â†’ 5x108; 2024 BMW M4 no-trim â†’ trimRequired; 2023 Ram 1500 no-trim â†’ not 325/65R18; 2020 Civic no-trim â†’ not Si envelope; ambiguity query throws (mock) â†’ non-certifiable.
  `/api/fitment/profile` same M4 / error cases.
  `/api/wheels/check-fitment` M4 no-trim + 5x120 SKU â†’ `fits:null, reason:"trim_required", boltPatternCompatible:true`; Fuel on Camry â†’ geometry_rejected unchanged.
  `/api/tires/search` 2020 Raptor 17" â†’ 110T tires `loadIndexOk:false`, no badge; packages â†’ no certified package containing a below-required tire.
- Run the whole existing suite before/after; report command + pass/fail counts.

## R6. Local verification and handoff
- PRECONDITION (supersedes the earlier "R1 done, start immediately" wording, which contradicted the outstanding SQL read-only / DB-cache-write items): the dev server may only be started once R1 is PROVEN on the exact env the server loads - `SHOW default_transaction_read_only` = `on` through POSTGRES_URL, a rolled-back `UPDATE ... WHERE false` rejected with "cannot execute UPDATE in a read-only transaction", `FITMENT_PREVIEW_READONLY=1` set (skips the `tireweb_sku_cache` upsert and the per-request `ensureFitmentTables` DDL), Redis vars stripped. Record the proof output in the progress file before the first request.
- Dev server on PORT 3002. Also hit `/api/wheels/fitment-search?year=2024&make=BMW&model=M4` (no trim -> every result `fitmentValidation.certified:false`, `certificationBlock:"trim_required"`, no surefit/specfit). Hit and record JSON for: `/api/vehicles/search?year=2024&make=Ford&model=Mustang`, `/api/wheels/check-fitment?sku=D68117906545&year=2024&make=Ford&model=Mustang`, `/api/vehicles/search?year=2024&make=BMW&model=M4`, `/api/wheels/check-fitment?year=2024&make=BMW&model=M4&sku=<a 5x120 sku from fitment-search>`, `/api/tires/search` for 2020 F-150 Raptor 17".
