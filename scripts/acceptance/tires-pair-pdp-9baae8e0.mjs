...
// 3c) (Codex review 17:47 on b1886aeb) the same sidebar priced a staggered WHEEL set as 4 x blended each:
//     RIDLER 2 x 336.12 + 2 x 345.41 = 1363.06 printed as 4 x 340.77 = 1363.08, wheel subtotal 2709.88 vs 2709.86.
//     Swap the square wheel for a staggered wheel line (shape the wheels PDP writes: rearSku + axle prices + rear fields)
//     plus a second square wheel, and require exact money on the wheel rows AND the wheel subtotal.
const ridler = { type: "wheel", sku: "RIDLER607-F", rearSku: "RIDLER607-R", brand: "Ridler", model: "607", finish: "Matte Grey", diameter: "19", width: "8.5", rearDiameter: "20", rearWidth: "10", quantity: 4, unitPrice: 340.77, frontUnitPrice: 336.12, rearUnitPrice: 345.41, staggered: true };
const square = { type: "wheel", sku: "TR04198551435BK", brand: "Touren", model: "TR04", finish: "Black", diameter: "19", width: "8.5", quantity: 4, unitPrice: 336.7, staggered: false };
...
await page.goto(`${BASE}${gtpp}`, { waitUntil: "networkidle2" });
const wheelLines = await page.$$eval('[data-testid="package-summary-wheel-line"]', (els) => els.map((e) => e.textContent.replace(/\s+/g, " ").trim()));
log("wheelSidebar.lines", wheelLines);
const ridlerExact = r2(2 * ridler.frontUnitPrice + 2 * ridler.rearUnitPrice); // 1363.06
const ridlerBlended = r2(4 * ridler.unitPrice); // 1363.08
const squareTotal = r2(4 * square.unitPrice); // 1346.80
const wheelSubtotalExact = r2(ridlerExact + squareTotal); // 2709.86
const wheelSubtotalBlended = r2(ridlerBlended + squareTotal); // 2709.88
const ridlerLines = wheelLines.filter((l) => l.includes("Ridler"));
check("wheelSidebar.staggeredExact2F2R", ridlerLines.length >= 1 && ridlerLines.every((l) => l.includes(`$${ridlerExact.toFixed(2)}`) || l.includes(`$${Math.round(ridlerExact)}`)), { ridlerExact, ridlerBlended, lines: ridlerLines.length });
check("wheelSidebar.staggeredShowsAxles", ridlerLines.some((l) => /Front .?2: 19x8\.5 . RIDLER607-F/.test(l) && /Rear .?2: 20x10 . RIDLER607-R/.test(l) && l.includes("2 \u00d7 $336.12 front + 2 \u00d7 $345.41 rear")), ridlerLines.find((l) => l.includes("Front")));
check("wheelSidebar.noBlendedWheel", !wheelLines.some((l) => l.includes(`$${ridlerBlended.toFixed(2)}`) || l.includes("4\u00d7 $340.77")), ridlerBlended);
check("wheelSidebar.squareUnchanged", wheelLines.filter((l) => l.includes("Touren")).every((l) => l.includes(`$${squareTotal.toFixed(2)}`) || l.includes(`$${Math.round(squareTotal)}`)), squareTotal);
const sidebarText = await page.$$eval("aside", (as) => as.map((a) => a.innerText.replace(/\s+/g, " ")));
log("wheelSidebar.subtotals", sidebarText.map((t) => (t.match(/Wheels[^$]{0,20}\$[\d,.]+/g) || []).join(" | ")));
check("wheelSidebar.subtotalExact", sidebarText.length >= 1 && sidebarText.every((t) => t.includes(`$${wheelSubtotalExact.toFixed(2)}`) && !t.includes(`$${wheelSubtotalBlended.toFixed(2)}`)), { wheelSubtotalExact, wheelSubtotalBlended });
const grand = r2(wheelSubtotalExact + setTotal);
check("wheelSidebar.grandTotalExact", sidebarText.every((t) => t.includes(`$${grand.toFixed(2)}`)), grand);

// 3d) SmartTireUpsell (package review, wheels-only cart) must not claim fit / no modifications.
const savedTireLines = await page.evaluate(() => {
  const all = JSON.parse(localStorage.getItem("wt_cart") || "[]");
  localStorage.setItem("wt_cart", JSON.stringify(all.filter((i) => i.type === "wheel")));
  return all.filter((i) => i.type !== "wheel");
});
await page.goto(`${BASE}/package/review`, { waitUntil: "networkidle2" });
await new Promise((r) => setTimeout(r, 4000));
const reviewBody = await page.evaluate(() => document.body.innerText.replace(/\s+/g, " "));
const upsellFacts = await page.$$eval('[data-testid="smart-tire-upsell-facts"]', (els) => els.map((e) => e.textContent.replace(/\s+/g, " ").trim()));
log("upsell.facts", upsellFacts);
check("upsell.noFitClaims", !/Fits your selected wheels|No modifications needed/.test(reviewBody), (reviewBody.match(/Complete Your Setup.{0,200}/) || ["(no upsell rendered)"])[0]);
check("upsell.neutralCopyWhenRendered", upsellFacts.length === 0 || upsellFacts.every((t) => t.includes("Fit not yet confirmed") && /Rim diameter matches your \d+" wheels/.test(t)), upsellFacts);

// restore the staggered tire pair for the cart/checkout sections below (they strip wheels themselves)
await page.evaluate((tires) => localStorage.setItem("wt_cart", JSON.stringify(tires)), savedTireLines);
await page.evaluate(() => {
  const c = JSON.parse(localStorage.getItem("wt_cart") || "[]").filter((i) => i.type !== "wheel");
  localStorage.setItem("wt_cart", JSON.stringify(c));
});
