...
for (const v of dbRows) {
  ...
  // route-level gate
  // year gate disabled for recovery estimate
...
fs.writeFileSync(path.join(__dirname, 'package-gap-trace-nogate.json'), JSON.stringify(results));
console.log('\nwrote package-gap-trace-nogate.json');
