...
for (const v of vehicles) {
  const rows = await sql`SELECT tire_size, front_tire_size, rear_tire_size, is_staggered, is_lifted, bolt_pattern
    FROM vehicle_fitments WHERE year = ${v.year} AND make = ${v.make} AND model = ${v.model} LIMIT 1`;
  if (rows[0]) {
    console.log(`${v.year} ${v.make} ${v.model}:`, JSON.stringify(rows[0]));
  } else {
