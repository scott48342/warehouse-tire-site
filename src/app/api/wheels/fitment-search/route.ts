...
function inferStaggeredFromTireSizes(tireSizes: string[]): StaggeredInfo | null {
  ...
  // Parse tire widths (e.g., "255/40R18" Ã¢â€ â€™ 255)
 ...
 * 2. If found Ã¢â€ â€™ use it directly (no legacy system needed)
 * 3. If not found Ã¢â€ â€™ fall back to legacy system (with logging)
...
export async function GET(req: Request) {
  ...
  if (vehicleIsDRWCapable) {
    console.log(`[fitment-search] Ã°Å¸â€ºÂ» HD TRUCK DETECTED: ${year} ${make} ${model}`, {
  ...
  try {
    ...
    if (modificationId || trimParam) {
      ...
      if (canonicalCheck.matchedBy === "blocked") {
        console.warn(`[fitment-search] Ã°Å¸Å¡Â« CANONICAL BLOCKED: ${year} ${make} ${model} Ã¢â€ â€™ ${canonicalCheck.debug.fallbackBlockedReason}`);
      ...
      if (canonicalCheck.matchedBy !== "not_found") {
        console.log(`[fitment-search] Ã¢Å“â€¦ CANONICAL RESOLVED: ${year} ${make} ${model} Ã¢â€ â€™ method=${canonicalCheck.matchedBy}, confidence=${canonicalCheck.confidence}, trim="${canonicalCheck.displayTrim}"`);
      }
    }
    ...
    // Primary path: Use modificationId-first lookup (DB Ã¢â€ â€™ Alias Ã¢â€ â€™ API)
    ...
    if (modificationId) {
      try {
        ...
        if (profileResult.profile) {
          ...
          console.log(`[fitment-search] RESOLVED (${resolutionPath}): ${year} ${make} ${model} mod=${modificationId}${aliasUsed ? ` Ã¢â€ â€™ ${canonicalModificationId}` : ''}`, {
      ...
      try {
        ...
        if (localFitments.length > 0) {
          ...
          if (bestFitment && bestFitment.boltPattern) {
            console.log(`[fitment-search] LOCAL DB HIT: ${year} ${make} ${model} Ã¢â€ â€™ ${bestFitment.modificationId} (boltPattern: ${bestFitment.boltPattern})`);
    ...
    if (!dbProfile && isClassicVehicle(Number(year), make)) {
      ...
      try {
        ...
        if (classicResult.isClassicVehicle && classicResult.fitmentMode === "classic") {
          console.log(`[fitment-search] CLASSIC FALLBACK HIT: ${year} ${make} ${model} Ã¢â€ â€™ platform=${classicResult.platform.code}`);
...
async function handleDbProfilePath(
  ...
  //   a) isStaggered=true  â†’ validated per-axle with requireAxleSpecific
  //   b) isStaggered=false AND confirmed square reason â†’ use DB midpoint (safe)
  // Any other "axle unknown" state â†’ hard fail.
  // ========================================================================
  if (isStaggeredCapableVehicle(make, model) && !staggeredInfo?.isStaggered) {
    ...
    if (!isConfirmedSquareSetup(reason)) {
      console.warn(
        ...
        `reason="${reason}" â€” failing closed`
      );
      logUnresolvedFitment({
        year, make, model,
        trim: dbProfile.displayTrim || undefined,
  ...
  // Null centerbore must not default to 0 â€” 0 passes every wheel's bore check.
  ...
  // Missing OEM offset â†’ no customer-facing wheel recommendations
  if (oemOffsetResult.missing) {
    console.warn(`[fitment-search] MISSING OEM OFFSET: ${year} ${make} ${model} mod=${canonicalModificationId || "(none)"} â€” ${oemOffsetResult.reason}`);
    logUnresolvedFitment({
      year, make, model,
      trim: dbProfile.displayTrim || undefined,
      searchType: "wheel",
  ...
  return await handleDbFirstWheelResults({
    ...
    dbProfileForResponse: {
      ...
      serviceSpecs: dbProfile.serviceSpecs ?? null,
      offsetRange: {
...
async function handleDbFirstWheelResults(opts: {
  ...
  dbProfileForResponse?: {
    ...
    serviceSpecs?: import("@/lib/fitment-db/profileService").FitmentServiceSpecs | null;
  ...
  console.log(`[fitment-search] Ã°Å¸â€Â SEARCHING: boltPattern=${opts.boltPattern}, rearWheelConfig=${opts.rearWheelConfig || 'n/a'}`);

  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Wheel-1 supplier (live, no gate) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  if (wheel1Candidates.length > 0) {
    console.log(`[fitment-search] Ã°Å¸Å¸Â¢ Wheel-1: ${wheel1Candidates.length} candidates (bp=${opts.boltPattern})`);
  }

  timing.candidatesDbMs = Date.now() - tCandidates0;
  
  console.log(`[fitment-search] Ã°Å¸â€œÂ¦ Found ${techfeedCandidates.length} WheelPros + ${wheel1Candidates.length} Wheel-1 candidates (bp=${opts.boltPattern})`);
  ...
  for (const c of diversifiedCandidates) {
    ...
    if (!hasUserOffsetFilter && wheelSpec.offset !== undefined) {
      ...
      if (Number.isFinite(wheelOffset)) {
        ...
        //   - High positive offset (Ã¢â€°Â¥+75mm) for front/inner positions
        //   - Extreme negative offset (Ã¢â€°Â¤-150mm) for outer positions
        // 
        ...
        if (opts.rearWheelConfig === "drw") {
          ...
          if (activeOemGeo && !activeOemGeo.missing) {
            ...
            if (geo.exceedsSafetyCeiling) {
              // Hard safety ceiling â€” always excluded in every profile
            ...
            if (!passesProfile) {
              ...
              // Falls into "extended" â€” passes but will be labeled as such by fitment guidance
            }
          }
        }
      }
  ...
  if (opts.rearWheelConfig === "drw" && drwDeadZoneExcluded > 0) {
    console.log(`[fitment-search] Ã°Å¸Å¡â€º DRW FILTER: Excluded ${drwDeadZoneExcluded} wheels with SRW-style offsets (-65 to +65mm)`);
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Wheel-1: inject synthetic inventory records (always) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
  ...
  // filter (NOT in cache Ã¢â€ â€™ EXCLUDED). Real feed replaces this in Phase 3.
  if (wheel1Candidates.length > 0) {
  ...
  // â”€â”€ WSI Wholesale: inject inventory so SKUs pass the ORDERABLE_TYPES filter â”€â”€
  ...
  // - If in inventory cache with qty >= 4 Ã¢â€ â€™ include
  // - If in inventory cache with qty < 4 Ã¢â€ â€™ exclude (can't fulfill a set)
  // - If NOT in inventory cache Ã¢â€ â€™ exclude (can't verify availability)
  //
  ...
  fitmentValidCandidates = fitmentValidCandidates.filter(item => {
    ...
    if (!inv) {
      if (debugSku && item.candidate.sku === debugSku) {
        debugTrace.push(`4. Inventory filter: NOT in cache Ã¢â€ â€™ EXCLUDED (can't verify stock)`);
      }
      return false;
    }
  ...
  if (debug && timing.inventoryFilteredOut > 0) {
    console.log(`[fitment-search] Ã°Å¸â€”â€˜Ã¯Â¸Â Inventory filter removed ${timing.inventoryFilteredOut} SKUs (not in feed or qty < ${MIN_INVENTORY_QTY})`);
  }
  ...
  // v3 ranking: TIER_1_BRANDS/TIER_2_BRANDS removed Ã¢â‚¬â€ replaced by fitmentClassScore
  ...
  const scoredCandidates: ScoredCandidate[] = fitmentValidCandidates.map(({ candidate: c, validation: v }) => {
    ...
    // Ã¢â€â‚¬Ã¢â€â‚¬ SCORING v3 (supplier-neutral, 2026-06-24) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    ...
    // 2. Fitment Class Score (0-100, weight: 20%)  Ã¢â€ Â replaces brandTierScore
    ...
    // 5. Price Range Score (0-100, weight: 10%)  Ã¢â€ Â reduced from 15%
    ...
    // 6. Customer Value Score (0-100, weight: 10%)  Ã¢â€ Â NEW in v3
    ...
    // Weighted total (v3 formula Ã¢â‚¬â€ weights from SCORE_WEIGHTS)
    const score = (
  ...
  // Ã¢â€ â€™ adds Rule 5 (supplier diversity cap) on top of existing rules 1-4
  ...
  if (applyPackagePriority && !isPriceSorted) {
    ...
    rankedCandidates = [...rankedCandidates].sort((a, b) => {
      ...
      // v3: package priority is supplier-neutral Ã¢â‚¬â€ image + stock matters, not supplier name
      ...
      // Sort by tier first (ascending: 1 Ã¢â€ â€™ 4)
      if (aTier !== bTier) {
        return aTier - bTier;
      }
    ...
    console.log(`[fitment-search] Ã°Å¸â€œÂ¦ PACKAGE PRIORITY applied: reordered ${rankedCandidates.length} results`);
  }
  ...
  if (opts.staggeredInfo?.isStaggered && opts.staggeredInfo.frontSpec && opts.staggeredInfo.rearSpec) {
    ...
    console.log(`[fitment-search] Ã¢Å“â€¦ STAGGERED PAIRING complete: ${staggeredPairsFound} pairs from ${styleGroups.size} styles`);
  ...
  // Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â
  // â”€â”€â”€ STAGGERED-ONLY MODE (staggeredOnly=1) â”€â”€â”€
  ...
  // â”€â”€â”€ DISJUNCTIVE SIZE FILTERING (Aug 2026) â”€â”€â”€
  ...
  // â”€â”€â”€ STYLE-GROUPED DIAMETER COUNTS (Aug 2026) â”€â”€â”€
  ...
  if (facets.wheel_diameter?.buckets) {
    ...
    const categorized = facets.wheel_diameter.buckets.map((bucket: { value: string; count: number }) => {
      ...
      return {
        ...
        label: isOem ? `${bucket.value}" Ã¢Ëœâ€¦` : `${bucket.value}"`,
      };
    });
...
async function handleLegacyPath(
  ...
  console.log(`[fitment-search] Ã¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢ÂÃ¢â€¢Â`);
  ...
  const profile = {
    ...
    fitment: {
      ...
      serviceSpecs: universalResult.serviceSpecs,
    },
  ...
  if (ruleOverride && ruleOverride.boltPattern && ruleOverride.boltPattern !== profile.boltPattern) {
    console.log(`[fitment-search] Ã°Å¸â€Â§ REDUNDANT RULE OVERRIDE (resolver should have handled this): ${year} ${make} ${model}`);
    console.log(`  Bolt pattern: ${profile.boltPattern} Ã¢â€ â€™ ${ruleOverride.boltPattern}`);
  ...
  const oemSpecs: OEMSpecs = {
    boltPattern: profile.boltPattern || "",
    // 2026-06-30: null centerbore must NOT become 72.6 fallback â€” fail closed instead.
  ...
  const legacyDbProfile = {
    ...
    serviceSpecs: profile.fitment?.serviceSpecs ?? null,
    offsetRange: {
  ...
  // Same rule as main path â€” must have confirmed square OR staggered+axle data.
  // ========================================================================
...
function buildFacets(wheels: any[]) {
  ...
  const normalizeBp = (bp: string) => String(bp || "").toLowerCase().replace(/[xÃƒâ€”-]/g, "x").trim();
