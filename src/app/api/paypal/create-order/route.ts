  ...
  try {
    ...
    try {
      ...
      if (!availCheck.ok && availCheck.unavailable) {
        ...
        console.warn("[paypal-checkout] Availability warning (not blocking):", availCheck.unavailable);
      }
    } catch (availErr) {
      console.warn("[paypal-checkout] Availability check error (not blocking):", availErr);
    }
