...
import { importVehicleFitment } from "../fitmentImport";
...
/**
 * Tier 3: Additional high-volume vehicles (remaining to reach 500+)
 */
...
// Years to import (most recent first for priority)
...
export function normalizeTrimDisplay(rawTrim: string | null | undefined): string {
  ...
  const noisePatterns = [
    /^\d+(\.\d+)?[LV]?\s*/i,           // Engine displacement at start "2.7L", "5.0 V8"
    ...
    /\bCVT\b/gi,
    /\bDCT\b/gi,
    /\s*\(\d{4}-\d{4}\)\s*/g,          // Year ranges "(2020-2024)"
    /\s*\(\d{4}\)\s*/g,                // Single year "(2024)"
    /\s+/g,                             // Multiple spaces
  ];
  ...
  // If we cleaned it down to nothing or just numbers, use "Base"
  if (!trim || /^\d+$/.test(trim)) {
    return "Base";
  }
...
export type CoverageStats = {
  ...
  estimatedSearchCoverage: number; // % of searches served without API
  estimatedApiReduction: number;   // % reduction in API calls
};
...
export async function calculateCoverage(): Promise<CoverageStats> {
  ...
  // Estimate search coverage based on vehicle sales distribution
  ...
  // API reduction = search coverage (remainder goes to API)
...
export type BulkImportConfig = {
  ...
  years: number[];
  dryRun?: boolean;
  delayBetweenCalls?: number; // ms between API calls
...
export async function executeBulkImport(
  ...
  const vehicles = getTargetVehicles(config);
  ...
  const delay = config.delayBetweenCalls ?? 500; // Default 500ms between calls

  for (const vehicle of vehicles) {
    const vehicleLabel = `${vehicle.year} ${vehicle.make} ${vehicle.model}`;
    progress.currentVehicle = vehicleLabel;
    onProgress?.(progress);
    ...
    if (config.skipExisting) {
      const existing = await db
        ...
        .where(
          and(
            eq(schema.vehicleFitments.year, vehicle.year),
            sql`LOWER(${schema.vehicleFitments.make}) = LOWER(${vehicle.make})`,
            sql`LOWER(${schema.vehicleFitments.model}) = LOWER(${vehicle.model})`
          )
        )
        .limit(1);
    ...
    if (config.dryRun) {
      console.log(`[DRY RUN] Would import: ${vehicleLabel}`);
      progress.success++;
      progress.completed++;
      onProgress?.(progress);
      continue;
    ...
    try {
      const result = await importVehicleFitment(vehicle.year, vehicle.make, vehicle.model, {
        usMarketOnly: true,
        debug: false,
      });
      ...
      if (result.success) {
        ...
        progress.failed++;
        progress.errors.push({ vehicle: vehicleLabel, error: result.error || "Unknown error" });
      ...
      progress.errors.push({ vehicle: vehicleLabel, error: errorMsg });
    ...
    const avgTimePerVehicle = elapsed / progress.completed;
    const remaining = vehicles.length - progress.completed;
    progress.estimatedTimeRemaining = Math.round((avgTimePerVehicle * remaining) / 1000);

    onProgress?.(progress);

    // Rate limiting
    if (delay > 0 && progress.completed < vehicles.length) {
      await new Promise(resolve => setTimeout(resolve, delay));
    }
  }

  progress.currentVehicle = null;
  onProgress?.(progress);

  // Calculate final coverage
  const coverageAfter = await calculateCoverage();

  return {
    success: progress.failed === 0,
    totalProcessed: progress.completed,
...
/**
 * Get full list of target vehicles (all tiers)
 */
